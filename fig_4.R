...
data_2024 <- combined_data %>%
  ...
  mutate(
    ...
    age = 2024 - as.numeric(as.character(`birth year`)),
    age_group = case_when(
      is.na(age) ~ "Missing",
      age < 35 ~ "18-34",
      age < 50 ~ "35-49", 
...
fraud_long_2024 <- data_2024 %>%
  select(all_of(fraud_columns), party, gender, race, education, age_group) %>%
  pivot_longer(cols = all_of(fraud_columns), names_to = "fraud_type", values_to = "belief") %>%
  filter(!is.na(belief))

fraud_long_2024 <- fraud_long_2024 %>%
  mutate(
